...
def inquiry(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question.save()
            messages.success(request, 'Inquiry Submission Successful. You will receive feedback via email soon.')
            form = InquiryForm()
    else:
        form = InquiryForm()
...
def privatemotor(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            private.save()
            messages.success(request, 'Private Motor Quotation Inquiry Submission Successful. You will receive feedback via email soon.')
            form = PrivateForm()
    else:
        form = PrivateForm()
...
def psv(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            bus.save()
            messages.success(request, 'PSV Quotation Inquiry Submission Successful. You will receive feedback via email soon.')
            form = PSVForm()
    else:
        form = PSVForm()
...
def commercial(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            comm.save()
            messages.success(request, 'Commercial Vehicle Quotation Inquiry Submission Successful. You will receive feedback via email soon.')
            form = CommercialForm()
    else:
        form = CommercialForm()
